...
// 이 예에서 알 수 있듯이 startWith는 선입 선출 방식(LastIn-FastOut)으로 연결될 수 있습니다. 즉, 각각의 연속되는 startWith의 요소는 이전 startWith의 요소 앞에 추가됩니다.
example("startWith") {
...
example("Observable startWith Vs. Observable concat") {
    let disposeBag = DisposeBag()
    ...
    // concat: 3, 4, 1, 2
    let first = Observable.of(3, 4)
    let second = Observable.of(1, 2)
    let observable = Observable.concat([first, second])
    observable.subscribe(onNext: { print($0) })
}
// Mathematical_and_Aggregate_Operators - concat(_:)
/*:
