...
example("of") {
    ...
    print("----------------< onNext: hello >----------------")
    Observable<String>.of("h", "e", "l", "l", "o")
        .subscribe(onNext: { print($0) })
        .disposed(by: disposeBag)
    
    print("----------------< subscribe: onNext: hello & completed >----------------")
    Observable<String>.of("h", "e", "l", "l", "o")
        .subscribe { print($0) }
        .disposed(by: disposeBag)
}
...
example("create") {
    ...
    let create = { (element: Int) -> Observable<Int> in
        return Observable.create { observer in
            ...
            observer.on(.next(element + 1)) // 4
            observer.on(.next(element + 10)) // 13
            observer.on(.next(element + 100)) // 103
            observer.on(.completed)
            return Disposables.create()
        }
    }

    create(3).subscribe({ (event) in
        print(event)
    }).disposed(by: disposeBag)
    
    create(4).subscribe(onNext: { print($0) })
        .disposed(by: disposeBag)
}
/*:
 ----
 ...
 */
// Defer 연산자는 관찰자가 구독 할 때까지 기다린 다음 Observable을 생성합니다. 일반적으로 Observable 팩토리 기능을 사용합니다. 각 구독자마다이 작업이 새로 수행되므로 각 구독자가 동일한 Observable을 구독하고 있다고 생각할 수도 있지만 실제로 각 구독자는 개별 시퀀스를 가져옵니다.
 ...
 */
// 생성된 각 이벤트에 대해 side-effect 액션을 호출하고 원래 이벤트를 반환합니다.
example("doOn") {
    let disposeBag = DisposeBag()
